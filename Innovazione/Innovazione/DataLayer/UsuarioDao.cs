...
namespace Innovazione.DataLayer
{
    class UsuarioDao
    {
        ...
        {
            string sql = "Select x.* from usuario x where x.nomUser = '" + nombre + "' AND contraseña = '" + password + "' AND x.estado = 'S'";
            DataTable oTabla;
        ...
        /*public int maximoID()
        {
            string sql = "SELECT MAX(U.idUsuario) as numero FROM Usuario U";
            int numero = Convert.ToInt32(BDHelper.getBDHelper().ConsultaSQL(sql));
            return numero;
        }¨*/
        ...
        public IList<Usuario> getByFilters(List<object> @params)
        {
            ...
            if (@params[1] != null)
            {
                if (@params[0] == null)
                {
                    sql += " u.estado LIKE @param2";
                }
                else
                {
                    sql += " AND u.estado LIKE @param2";
                }
        ...
        public bool create(Usuario oUsuario)
        {
            ...
            sql = "INSERT INTO usuario (nomUser, contraseña, estado) VALUES('";
        ...
        public bool update(Usuario oUsuario)
        {
            ...
            sql += oUsuario.contraseña + "', estado = '";
            sql += oUsuario.activo + "'";   
        ...
        private Usuario map_user(DataRow row)
        {
            ...
            oUsuario.activo = row["estado"].ToString();
            return oUsuario;
        }
    }
}
